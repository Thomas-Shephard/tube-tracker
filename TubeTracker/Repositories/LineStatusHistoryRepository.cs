...
{
    public async Task UpsertAsync(int lineId, int statusSeverity)
    {
        ...
        int? historyId = await connection.QueryFirstOrDefaultAsync<int?>(findQuery, new { LineId = lineId, StatusSeverity = statusSeverity, Threshold = threshold });

        if (historyId.HasValue)
        {
            const string updateQuery = "UPDATE LineStatusHistory SET last_checked_at = @Now WHERE history_id = @HistoryId";
            await connection.ExecuteAsync(updateQuery, new { Now = DateTime.UtcNow, HistoryId = historyId.Value });
        }
        else
        {
            const string insertQuery = "INSERT INTO LineStatusHistory (line_id, status_severity, first_checked_at, last_checked_at) VALUES (@LineId, @StatusSeverity, @Now, @Now)";
    ...
    {
        const string query = """
                             SELECT lsh.history_id, lsh.line_id, lsh.status_severity, lsh.first_checked_at, lsh.last_checked_at,
                             ...
                             WHERE lsh.line_id = @LineId 
                               AND lsh.last_checked_at >= @Threshold
                             ORDER BY lsh.last_checked_at DESC
                             """;
        ...
        return rows.Select(row => new LineStatusHistory
        {
            ...
            FirstCheckedAt = (DateTime)row.first_checked_at,
            LastCheckedAt = (DateTime)row.last_checked_at,
